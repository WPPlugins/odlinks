...
function odl_edit_category($id) {
	...
	<label>Parent category:</label>
		...
		<?php odl_list_cats(0,0,$odCategories['c_id'],$odCategories['c_parent']); ?>
		</select>
	<br>
...
function odl_view_category($id) {
	...
	<tr><td> 
		...
		<tr style="background-color:#ccc">
			<td class="struct"><img border=0 src="<?php echo get_bloginfo('wpurl'); ?>/wp-content/plugins/odlinks/images/edit.gif"> Title / Name</td>
			<td class="struct" width="150">Number of links</td>
			<td nowrap class="struct">Date</td>
			<td calss="struct">Delete</td>
			</tr> 
...
function odl_list_cats($parent, $lev, $exclude, $selected) { 
	...
	for ($i=0; $i<count($categories); $i++){
		...
		$res = $wpdb->get_row("SELECT * FROM {$table_prefix}odlinks WHERE l_c_id = '".$category->c_id."'", ARRAY_A); 
		$linksNum = count($res);
		$id = $category->c_id; 
...
function odl_main_cats($parent,$lev,$orderby,$how) { 
	...
	for ($i=0; $i<count($categories); $i++){
		...
		?>
		<td style="background-color:#E9E9E9"><a style="text-decoration: none;" href="javascript:deleteCategory('<?php echo rawurlencode($category->c_title . " " . $category->c_name );?>', '<?php echo $PHP_SELF;?>?page=odlinksstructure&odlinks_admin_action=deleteCategory&c_id=<?php echo $id;?>');"><img border=0 src="<?php echo get_bloginfo('wpurl'); ?>/wp-content/plugins/odlinks/images/delete.png"></a></td></tr>
		<?php
