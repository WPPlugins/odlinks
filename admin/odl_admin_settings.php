...
function process_odlinkssettings(){
	...
	switch ($_GET['odlinks_admin_action']){
		...
		foreach ($_POST["odlinksdata"] as $k=>$v){
			$_POST["odlinksdata"][$k] = stripslashes($v);
		}
		...
		$odlinkssettings = get_option('odlinksdata');
		$odl_slug_check = $wpdb->get_results("SELECT post_name FROM {$table_prefix}posts WHERE post_name = '".$wpdb->escape($odl_new_slug)."'");
		$odl_slug_checked = $odl_slug_check[0];
		...
		if($odl_new_slug!=$odlinkssettings['odlinksslug']){
			if($odl_slug_checked->post_name!=$odl_new_slug){
				$wpdb->query("UPDATE {$table_prefix}posts SET post_name = '".$odl_new_slug."' WHERE post_title = '[[ODLINKS]]'");
				$msg ="Settings Updated.";
			}else{	
				$msg ="Settings Updated but a Slug exists with the name: ".$odl_new_slug."<br />"."Try Again.";
				$_POST['odlinksdata']['odlinksslug'] = $odlinkssettings['odlinksslug'];
			}
		}

		$odlinksnew_settings = array();
		$odlinksnew_settings = $_POST['odlinksdata'];
		update_option('odlinksdata', $odlinksnew_settings);
	...
	<p>
	<form method="post" id="odlSettings" name="odlSettings" action="<?php echo $PHP_SELF;?>?page=odlinks&odlinks_admin_action=savesettings">
		<input type="hidden" name="odlinksdata[odlinksversion]" value="<?php echo $odlinksversion;?>">
		<table border="0" class="editform">
			<tr>
			<th align="right">odlinks Version:</th>
			<td><?php echo $odlinksversion;?></td>
			</tr>
			<tr><th align="right">WordPress Version:</th>
			<td><?php echo $wp_version;?></td>
			</tr>
			<tr><th align="right">odlinks URL:</th>
			...
			<tr><th align="right">Excerpt Length:</th>
			...
			<tr><th align="right">Display # of new/updated links:</th>
			...
			<tr><th align="right">of subcategories under each category link:</th>
			...
			<tr><th align="right">of subcategories to display under each category:</th>
			...
			if (!isset($odlinkssettings['googleID'])) $odlinkssettings['googleID'] = 'pub-2844370112691023';
			...
			<td><input type='text' name='odlinksdata[googleID]' id='odlinksdata[googleID]' value="<?php echo ($odlinkssettings['googleID']);?>" size='30' /><br><span class="smallTxt"> example: no, pub-2844370112691023 or ...
			</span></td></tr>
			<tr>
...
function odlinksinstall(){
	...
	if ($odlinkssettings['odlinksinstalled']!='y'){
		...
		$odlinkssettings['odlinks_last_links_num'] = 5;
		$odlinkssettings['odlinks_excerpt_length'] = 100;
...
//		
		...
		$odlinkssettings['googleID'] = 'pub-2844370112691023';
		$odlinkssettings['GADproduct'] = 'link';
