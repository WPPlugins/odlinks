...
function process_odlinksposts(){
	...
	<input type="button" value="Approve" onclick="document.location.href='<?php echo $PHP_SELF;?>?page=odlinksposts&odlinks_admin_action=approvelinks&id=0';">

	<input type="button" value="Banned" onclick="document.location.href='<?php echo $PHP_SELF;?>?page=odlinksposts&odlinks_admin_action=bannedlinks&id=0';">
	...
	if(isset($id) && $action){ 
		switch($action){ 
			case "ok": 
				...
				for($i=0; $i<count($news); $i++){
					...
					<input type="hidden" name="odlinksdata[new_link]" value="1"> <?php $warning?> URL:<br>
					<input type="text" name="odlinksdata[url]" value="<?php echo $new->n_url?>" size="80"> 
						<a target="_blank" href="<?php $new->n_url?>"><img src="<?php echo get_bloginfo('wpurl'); ?>/wp-content/plugins/odlinks/images/nw2.png"></a> <p> Title:<br>
					<input type="text" name="odlinksdata[title]" value="<?php echo $new->n_title; ?>" size="80"> <p> Email:<br>
					<input type="text" name="odlinksdata[email]" value="<?php echo $new->n_email; ?>" size="80"> 
						...
						<select name="odlinksdata[parent]"> 
							<?php
							odl_list_cats(0,0,0,$new->n_category);
							?> </select> <p> 
		...
		if(!empty($results)){
			...
			for($i=0; $i<count($results); $i++){
				...
				odl_list_cats(0,0,0,$row->n_category);
	...
	if(isset($id) && $action){ 
		...
		if(!empty($results)) {
			for($i=0; $i<count($results); $i++){
				...
				<br><label>Category:</label><select name="odlinksdata[parent]"> <?php odl_list_cats(0,0,0,$row->l_c_id); ?> </select>
				<P><label>&nbsp;</label>
