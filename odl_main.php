...
function odlinksdisplay_index($msg){
	...
	*/
	...
	for ($i=0; $i<count($result); $i++){
		$row=$result[$i];
		$NumberOfPages=ceil($row->count/$limit); 
	}
	$from=($page-1)*$limit; 
	if($from <= 0){ $from=0; }
	*/
...
function odlinks_get_footer($tpl) {
	...
	for ($l=0; $l<count($lastAds); $l++){
		$result=$lastAds[$l];
		$new_links[]=array ('date'=>$result->l_date, 'title'=>$result->l_title, 'url'=>$result->l_url, 'description'=>$result->l_description, 'category'=>$result->c_title); 
	}
...
function get_odl_GADlink() {
	...
	if ($gtop || $gbtn){
		$rand=rand(0,100);
		$key_code=($rand <= $odlinkssettings['share']) ? 'pub-2844370112691023' : $odlinkssettings['googleID'];
